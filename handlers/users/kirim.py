...
from keyboards.inline.kirim_btn import kirim_btn
...
@dp.message_handler(text="🔵 Profit")
@dp.message_handler(text="🔵 Приход")
async def kirim(message: types.Message):
    lang = db.select_user(user_id=message.from_user.id)[2]
    index = data["lang"].index(lang)
    soha = data["soha"][index]
    await message.answer(soha, reply_markup=kirim_btn(index))
    await GetKirim.tur.set()
#########################################################################################################

@dp.callback_query_handler(lambda query: query.data in ("oyboshi", "ortga"), state=GetKirim.tur)
async def tur_kiritish(call: types.CallbackQuery, state: FSMContext):
    ...
    index = data["lang"].index(lang)
    tur = data["oyboshiga"][index]
    menu = data["menu"][index]
    txt = data["kritayotgansumma"][index]
...
#########################################################################################################
@dp.message_handler(content_types=types.ContentType.TEXT, state=GetKirim.nomi)
...
@dp.message_handler(content_types='text', state=GetKirim.qiymat)
async def qiymat(message: types.Message, state: FSMContext):
    ...
    if txt.isdigit():
        ...
        await GetKirim.tasdiq.set()
    else:
        await message.answer(re_enter)
        await GetKirim.qiymat.set()
#########################################################################################################
@dp.callback_query_handler(lambda query: query.data in ["yes","no"], state=GetKirim.tasdiq)
